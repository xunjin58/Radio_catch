...
AI 混剪规划使用 `remix_planning` 任务模型；如未单独分配，则使用默认且已启用图片输入的模型。规划仅读取同菜品、已审核素材，并临时发送每条候选的封面和最多 3 张关键帧。图片数据、原视频和 API Key 都不得写入日志、数据库、响应或故障单。素材库较大时服务端只发送 24 条代表性候选；规划数少于请求数量属于正常结果，表示当前素材不足以形成更多不重复变体。
