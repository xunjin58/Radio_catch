...
| `RADIO_CATCH_PROJECT_DIR` | 推荐的单一项目文件夹；标签数据库、素材/派生帧和导出均归入其中。 | 未设置，使用旧目录布局 |
...
设置 `RADIO_CATCH_PROJECT_DIR`（例如 Windows 的 `D:\RadioCatch\lemon-september`）后，系统固定使用 `<项目>/radio_catch.db`、`<项目>/media/` 和 `<项目>/exports/`；显式设置的旧路径变量仍优先，以兼容现有部署。不要将同一个项目的三类路径再分别设到不同磁盘目录。

将旧工作区收敛为项目文件夹时，在项目源码根目录执行：
...
脚本会复制标签数据库、原始素材、派生帧和全部导出/manifest，并在**副本**的数据库中改写本机绝对路径；旧目录不会删除。将整个项目文件夹复制到另一台电脑后，首次启动会对其中实际存在文件的素材、标签证据、EDL 和交付路径自动做安全重定位。确认新目录能播放素材、查看标签和导出后，才可再次运行并加 `--move` 清理旧运行时数据。迁移到另一台电脑后，不要复制 `.env`；以新的本机密钥启动，并在 Web 中重新保存模型 API Key。
...
素材库详情播放使用 `GET /api/media/clips/<clip_id>/video`。它只会提供仍位于 `RADIO_CATCH_PROJECT_DIR/media`（或显式 `RADIO_CATCH_STORAGE_DIR`）内、且已在数据库登记的原视频；若详情显示视频无法播放，先确认原文件未被手动移动或删除，并确认浏览器支持该视频编码。
...
成片管理使用 `GET /api/renders/<render_id>/video` 播放最终 MP4，并通过 `GET /api/renders/<render_id>/thumbnail` 读取宫格封面。封面由 FFmpeg 从最终成片约 15% 时刻提取，存放在项目 `exports/`（或 `RADIO_CATCH_EXPORT_DIR`）中，属于可再生派生文件；历史成片首次请求封面时会自动补生成。两个接口都只允许读取已完成且仍位于导出目录内的成片。
...
按 [MiMo 音画后期流程](mimo-postproduction.md)执行。每个批次都在 `RADIO_CATCH_PROJECT_DIR/exports`（或 `RADIO_CATCH_EXPORT_DIR`）下新建独立目录，绝不覆盖基础 Render 或旧版后期文件。
...
| 素材详情视频返回 404 或无法播放 | 确认素材 ID 存在、原文件仍在项目 `media/`（或 `RADIO_CATCH_STORAGE_DIR`）内；若刚完成迁移，确认启动 shell 已设置 `RADIO_CATCH_PROJECT_DIR`，且迁移脚本已完成；若文件存在但浏览器不支持编码，重新导入兼容编码的 MP4。 |
...
| 导出失败 | 检查每段完整原视频是否已审核、同菜品、具有有效时长，及项目 `exports/`（或 `RADIO_CATCH_EXPORT_DIR`）是否可写。 |
| 下载成片返回 409/404 | 先确认成片状态为 `completed`，并确认导出文件仍在项目 `exports/`（或 `RADIO_CATCH_EXPORT_DIR`）中。 |
| 最终交付版未显示或返回 409/404 | 确认 Agent 后期命令完整成功、`--music-path` 和授权引用已提供，并检查最终 MP4 仍位于项目 `exports/`（或 `RADIO_CATCH_EXPORT_DIR`）；重新运行同批后期会回写对应 Render，基础 Render 不受影响。 |
| 成片宫格没有封面 | 确认 FFmpeg 可用、成片文件仍在项目 `exports/`（或 `RADIO_CATCH_EXPORT_DIR`）且可解码；刷新宫格会再次请求并补生成封面。封面失败不影响 MP4 下载或播放。 |
| CSV 跳过数据 | 查看导入响应中的 `errors`；确认 `video_id` 是本系统 Render 返回的值。 |

项目运行时数据未纳入版本控制。使用项目文件夹时，升级前备份整个 `RADIO_CATCH_PROJECT_DIR`；旧布局则备份 `backend/data/` 和 `backend/storage/`。
