...
2. 按镜头分段编写口播，使最终 TTS 从视频开头连续覆盖至结尾；默认使用当前批准的 MIMO“茉莉”音色和 1.2 倍语速，用户另有指定时以其为准。
...
2. **看片与文案**：使用 MiMo `mimo-v2.5` 原生视频审核（2 fps、JSON）；仅保存分段可见事实。人工文案逐句映射镜头，商业背景和模型推断不能充当产地、价格、无籽、营养或功效卖点的依据。
