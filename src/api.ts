...
export type Clip = {
  ...
  climax_time: number | null; confidence: number | null; duration_seconds: number | null; usable_range: { start: number; end: number } | null; created_at: string
}
export type Render = {
...
export type ExperimentRequest = { name: string; dish: string; target_duration_seconds: number; generation_count: number; variables: Record<string, unknown>; variants: Array<{ name: string; clips: Array<{ clip_id: string }>; values?: Record<string, unknown> }> }
